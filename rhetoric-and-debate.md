...
Related is the tactic of *moving the goalposts*. Moving the goalposts occurs when a speaker holds a position justified by a fallacious argument, and when said argument is disproven, refuses to accept it, instead creating a new argument that justifies an easier-to-defend position. The new argument is often fallacious itself, and so the cycle can continue. The reason why this is wrong is because, if the argument was occurring in good faith, the speaker would begin with their actual position and strongest argument for it. 
